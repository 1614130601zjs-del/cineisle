...
public class MainActivity extends Activity {
    ...
    private TextView fullscreenSubtitleOverlay = null;
    ...
    private boolean requestingProjection = false;
...
private final Runnable poller = new Runnable() {
    ...
    private LinearLayout buildRoom() {
        ...
        tabBar.setPadding(0, dp(4), 0, dp(4));
        Button tabChat = btn("聊天", true);
        Button tabSense = btn("感知", false);
        Button tabNote = btn("笔记", false);
        Button tabHall = btn("影厅", false);
        tabBar.addView(tabChat, new LinearLayout.LayoutParams(0, dp(40), 1));
        tabBar.addView(tabSense, new LinearLayout.LayoutParams(0, dp(40), 1));
        tabBar.addView(tabNote, new LinearLayout.LayoutParams(0, dp(40), 1));
        tabBar.addView(tabHall, new LinearLayout.LayoutParams(0, dp(40), 1));
        add(c, tabBar, -1, 40, 10);

        FrameLayout tabContent = new FrameLayout(this);
        tabContent.setLayoutParams(new LinearLayout.LayoutParams(-1, 0, 1));

        // Chat Tab
        LinearLayout chatTab = panel();
        chatTab.addView(tv("岛上留言与弹幕雨", 18, Typeface.BOLD));
        chatTab.addView(small("聊天像留言，弹幕像漂过银幕的小纸条，都会进入本场观影时间轴。"));
        ...
        add(chatTab, chatLog, -1, 170, 10);
        chatInput = input("今晚想和对方说什么？", "");
        add(chatTab, chatInput, -1, 48, 10);
        ...
        add(chatTab, cr, -1, 44, 8);
        tabContent.addView(chatTab);

        // Sense Tab
        LinearLayout senseTab = panel();
        senseTab.setVisibility(View.GONE);
        senseTab.addView(tv(aiName() + "感知", 18, Typeface.BOLD));
        senseTab.addView(small("同步片名、进度、当前字幕和最近字幕；低频画面截图可交给无障碍服务自动上传，只在你打开开关后启用。"));
        ...
        add(senseTab, contextState, -1, -2, 10);
        ...
        add(senseTab, senseRow1, -1, 44, 10);
        ...
        add(senseTab, senseRow3, -1, 44, 8);
        tabContent.addView(senseTab);

        // Note Tab
        LinearLayout noteTab = panel();
        noteTab.setVisibility(View.GONE);
        noteTab.addView(tv("时间轴笔记", 18, Typeface.BOLD));
        noteTab.addView(small("记录某一幕、摘一句台词，最后自动落进票根和档案馆。"));
        noteInput = input("这一幕想记下什么？也可以直接写一句台词", "");
        noteInput.setSingleLine(false);
        add(noteTab, noteInput, -1, 88, 10);
        ...
        add(noteTab, noteButtons, -1, 44, 10);
        ...
        add(noteTab, noteLog, -1, 180, 10);
        tabContent.addView(noteTab);

        // Hall Tab (影厅)
        LinearLayout hallTab = panel();
        hallTab.setVisibility(View.GONE);
        hallTab.addView(tv("观影邀请卡", 17, Typeface.BOLD));
        ...
        add(hallTab, inviteSummary, -1, -2, 10);
        ...
        final View[] allPanels = {chatTab, senseTab, noteTab, hallTab};
        final int[] activeTabIdx = {0};
        android.view.View.OnClickListener tabClick = v -> {
            int idx = -1;
            if (v == tabChat) idx = 0;
            else if (v == tabSense) idx = 1;
            else if (v == tabNote) idx = 2;
            else if (v == tabHall) idx = 3;
            if (idx < 0) return;
            activeTabIdx[0] = idx;
            for (int i = 0; i < allTabs.length; i++) {
                boolean active = (i == idx);
                allTabs[i].setBackground(active ? grad(new int[]{accent(), accent2()}, 18) : round(cardSoft(), 18));
                allTabs[i].setTextColor(active ? Color.WHITE : ink());
                allPanels[i].setVisibility(active ? View.VISIBLE : View.GONE);
            }
        };
        tabChat.setOnClickListener(tabClick);
        tabSense.setOnClickListener(tabClick);
        tabNote.setOnClickListener(tabClick);
        tabHall.setOnClickListener(tabClick);

        // Also need to move inviteMini out of direct add(c, ...) - remove it from header section
        ...
        contextToggle.setOnClickListener(v -> {
            ...
            savePrefs();
            updateServicePrefs();
            contextToggle.setText(contextAutoSync ? "字幕感知 ON" : "字幕感知 OFF");
            contextToggle.setBackground(contextAutoSync ? grad(new int[]{accent(), accent2()}, 18) : round(cardSoft(), 18));
        ...
        screenshotToggle.setOnClickListener(v -> {
            ...
            savePrefs();
            updateServicePrefs();
            screenshotToggle.setText(autoScreenshot ? "自动截图 ON" : "自动截图 OFF");
            screenshotToggle.setBackground(autoScreenshot ? grad(new int[]{accent(), accent2()}, 18) : round(cardSoft(), 18));
            if (contextState != null) contextState.setText(contextStatusText());
    ...
    private void rebuild() {
        String savedPage = currentPage;
        root.removeAllViews();
        buildUI();
        updateHeroTexts();
        showPage(savedPage.length() > 0 ? savedPage : (roomId.length() > 0 ? "room" : "home"));
        renderCard();
    }
    ...
    private void updateViewingContext(boolean force) {
        ...
        if (fullscreenSubtitleOverlay != null) {
            fullscreenSubtitleOverlay.setText(cur);
            fullscreenSubtitleOverlay.setVisibility(cur.length() > 0 ? View.VISIBLE : View.GONE);
        }
        if (contextState != null) contextState.setText(contextStatusText());
        updateServicePrefs();
    }
    ...
    private void openCinemaFullscreen() {
        ...
        // Fix 3: Scale video to fill screen (crop to fit)
        host.post(() -> {
            try {
        ...
        // Fix 5: Fullscreen subtitle overlay
        fullscreenSubtitleOverlay = small("");
        fullscreenSubtitleOverlay.setTextColor(Color.WHITE);
        fullscreenSubtitleOverlay.setGravity(Gravity.CENTER);
        fullscreenSubtitleOverlay.setTextSize(15);
        fullscreenSubtitleOverlay.setPadding(dp(12), dp(8), dp(12), dp(8));
        fullscreenSubtitleOverlay.setBackground(round(color("#99000000"), 18));
        fullscreenSubtitleOverlay.setVisibility(View.GONE);
        FrameLayout.LayoutParams fsLp = new FrameLayout.LayoutParams(-1, -2, Gravity.BOTTOM | Gravity.CENTER_HORIZONTAL);
        fsLp.setMargins(dp(16), 0, dp(16), dp(60));
        host.addView(fullscreenSubtitleOverlay, fsLp);
        ...
        d.setOnDismissListener(x -> {
            ...
            try {
                ...
                fullscreenSubtitleOverlay = null;
